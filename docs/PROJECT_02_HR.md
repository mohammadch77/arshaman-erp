...
۶. **تاریخ‌ها همیشه شمسی‌اند، هیچ‌وقت میلادی خام روی صفحه یا در ورودی** — طبق بخش ۳ CLAUDE.md
   («ذخیره UTC، نمایش و ورودی شمسی»). ذخیره در دیتابیس همیشه میلادی/UTC می‌ماند (بدون تغییر معماری)؛
   فقط لایه Blade/Livewire شمسی است. الگوی مرجع از Session 1: `App\Support\Jalali` (تبدیل دوطرفه +
   کلمپ‌کردن روزهای نامعتبر) و کامپوننت `<x-jalali-date-select field="..." />` (سه انتخاب‌گر
   سال/ماه/روز شمسی، بدون افزودن پکیج جدید). این الگو را در Session های بعدی (تقویم کاری، حضور و غیاب،
   مرخصی، حقوق) هم تکرار کن، نه یک‌بارمصرف برای Employees.

   **چک‌لیست پایانی الزامی هر Session HR از این به بعد:**
   «آیا همه تاریخ‌های ورودی/نمایشی این Session شمسی‌اند؟» — قبل از اعلام پایان هر Session HR (تقویم
   کاری، حضور و غیاب، مرخصی، حقوق)، این سؤال را صریح بپرس و جواب بده، دقیقاً مثل چک‌لیست تم Mary UI.
