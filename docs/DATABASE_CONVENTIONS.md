...
> ⚠️ **به‌روزرسانی (تصمیم کارفرما، مستند در `DECISIONS.md`):** از ماژول
> انبار (فاز ۳) به بعد، enum های کسب‌وکاری بسته و کنترل‌شده توسط
> توسعه‌دهنده باید **ENUM نیتیو MySQL** باشند، نه VARCHAR+CHECK. قانون
> اجباری: مقادیر جدید فقط به انتهای لیست اضافه شوند، هرگز ترتیب موجود
> عوض یا مقداری حذف نشود. استثنا: مقادیر باز که کاربر نهایی (نه
> توسعه‌دهنده) باید بتواند بدون تغییر کد مقدار جدید اضافه کند (مثل
> `shipments.carrier`) همچنان VARCHAR می‌مانند. ماژول‌های قبل از این
> تاریخ (Auth، HR، Core، CRM) با قرارداد قدیم (VARCHAR+CHECK) باقی
> می‌مانند تا Session تبدیل عقب‌رونده (نگاه کن `BACKLOG.md`) اجرا شود.
...
## ۱۷. سخت‌گیری مرجعی — قانون دائمی از این به بعد

طبق تصمیم صریح کارفرما بعد از بازطراحی کامل دیتابیس ماژول Process (بند ۱۶
بالا)، این قانون از این پس برای **کل پروژه** لازم‌الاجراست، نه فقط ماژول
Process:

۱. **هیچ ستون JSON برای داده‌ای که رابطه (relation) دارد یا ساختار
   مشخص/تکرارشونده دارد استفاده نشود** — چنین داده‌ای باید در یک جدول واقعی
   با کلید خارجی (Foreign Key) نگه‌داری شود، نه به‌صورت بلوب JSON با کلید
   قراردادی. JSON فقط برای داده‌ی واقعاً آزاد/غیرقابل‌پیش‌بینی (مثل تنظیمات
   نمایشی صرف) مجاز است، نه برای چیزی که باید به رکورد دیگری اشاره کند.

۲. **رابطه‌ی پلی‌مورفیک فقط زمانی استفاده شود که واقعاً از پیش مشخص نیست
   یک رکورد به کدام نوع والد تعلق دارد** (مثل ویجت فرم که می‌تواند مال یک
   definition یا یک step باشد). اگر یک جدول همیشه دقیقاً یک نوع والد ثابت
   دارد، باید دو جدول جدا با FK مستقیم و واقعی ساخته شود، نه یک جدول
   پلی‌مورفیک مشترک با ابهام نوع.

۳. **هرجا یک ستون باید به مقدار مشخصی در جدول دیگر اشاره کند، از FOREIGN
   KEY واقعی استفاده شود**، نه یک VARCHAR/رشته‌ی آزاد که فقط در لایه‌ی
   برنامه اعتبارسنجی می‌شود.

این قانون از این پس در تمام migration های جدید کل پروژه (نه فقط ماژول
Process) باید رعایت شود، حتی بدون یادآوری صریح کارفرما در هر Session.

**مرجع نمونه:** بازطراحی کامل دیتابیس ماژول Process (تبدیل
`request_form_fields`/`step_form_fields`/`request_data`/`step_data` از JSON
به جداول `process_form_fields`/`process_instance_field_values`/
`process_instance_log_field_values` با کلید خارجی واقعی).
