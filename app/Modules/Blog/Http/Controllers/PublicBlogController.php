 ...
 * withoutGlobalScopes() می‌شود و owner_company_id از companySlug مسیر گرفته
...
class PublicBlogController extends Controller
{
    public function index(string $companySlug, Request $request): View
    {
        ...
        $categories = BlogCategory::withoutGlobalScopes()
    ...
    public function show(string $companySlug, string $postSlug): View
    {
        ...
        $relatedPosts = BlogPost::withoutGlobalScopes()
            ->where('owner_company_id', $company->id)
            ->where('id', '!=', $post->id)
